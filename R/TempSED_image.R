...
image2D.TempSEDdyn <- function (z, which = NULL, subset = NULL,
    ...
    xlab = time_unit, time_origin = NULL, plotWater = FALSE,
...
image2D.TempSED1D <- function (z, which, colkey, time_origin, plotWater, ...)
{
  x  <- z
  ...
  for (ip in 1:nv) {
    ...
    if (plotWater & Which[ip] == "Temperature"){  # add water height/plotWater temp
      BW.temp   <- subset(x, which = "Watertemperature")
      BW.height <- -pmax(0, subset(x, which = "Waterheight"))
      z         <- cbind(BW.temp, z)
      BWy       <- matrix(nrow = length(times), ncol = length(grid),
                       data = grid, byrow = TRUE)
  ...
  if (n2D > 0) { # 2D variable
    ...
    for (i in 1:nrow(x)) {
     for (ip in 1:length(Which)) {
       ...
       if (plotWater & Which[ip] == "Temperature"){
