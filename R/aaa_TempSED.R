...
.TMPSED$parms <- c(
   ...
   kd_water        = 1.0,   # [/m] light attenuation coefficient water (1-25/m in RT13)
   kd_sediment     = 1000,  # [/m] light attenuation coefficient (bulk) sediment
   cp_water        = 3994,  # [J/kg/dgC] specific heat capacity water
   cp_solid        = 1000,  # [J/kg/dgC] specific heat capacity solid (dry sediment)
   tc_water        = 0.6,   # [W/m/dg]   thermal conductivity of water 
   tc_solid        = 2.1,   # [W/m/dg]   thermal conductivity of solid* 
...
.TMPSED$out0D_description <- c( 
             ...
             Airhumidity         = "Relative humidity of air",
...
.TMPSED$forc_names <- c("f_Waterheight",    "f_Watertemperature", 
                        "f_Airtemperature", "f_Airhumidity",
                        "f_Pressure",      "f_Solarradiation",    
                        "f_Windspeed" ,    "f_Cloudiness")

.TMPSED$forc_units <- c("m", "dgC", "dgC", "-",
                        "Pa", "W/m2", "m/s", "-")
