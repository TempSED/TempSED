...
* the specific air humidity ($kg~water(kg~air)^{-1}$), 
...
Vtq <- outer(X   = T.seq, 
             Y   = Q.seq, 
             FUN = function(Ta, Q) 
               air_properties(T_air = Ta, Qrel = Q)$Pvapor)
Vtp <- outer(X   = T.seq, 
             Y   = P.seq, 
             FUN = function(Ta, P) 
               air_properties(T_air = Ta, P = P)$Pvapor)
Htq <- outer(X   = T.seq, 
             Y   = Q.seq, 
             FUN = function(Ta, Q) 
               air_properties(T_air = Ta, Qrel = Q)$Qspec)
Htp <- outer(X   = T.seq, 
             Y   = P.seq, 
             FUN = function(Ta, P) 
               air_properties(T_air = Ta, P = P)$Qspec)
``` 
...
image2D(z = Dtq, x = T.seq, y = Q.seq, 
        contour = TRUE, 
        main = "density", xlab = "T_air, degC", 
        ylab = "Qrel, -", clab = "kg/m3")
image2D(z = Vtq, x = T.seq, y = Q.seq, 
        contour=TRUE, 
        main = "vapor pressure", xlab = "T_air, degC",  
        ylab = "Qrel, -", clab = "Pa")
image2D(z = Htq, x = T.seq, y = Q.seq, 
        contour=TRUE, 
        main = "specific air humidity", xlab = "T_air, degC", 
...
* saturated specific humidity (kgWater/kgAir): from https://www.engineeringtoolbox.com/humidity-ratio-air-d_686.html
...
image2D(z = Dtq,  x = T.seq, y = S.seq, 
        contour = TRUE, 
        main = "density", xlab = "T_water, degC", 
        ylab = "S, -", clab = "kg/m3")
image2D(z = CPtq, x = T.seq, y = S.seq, 
        contour = TRUE, 
        main = "specific heat capacity", xlab = "T_water, degC", 
        ylab = "S -", clab = "J/kg/K")
image2D(z = TDtq, x = T.seq, y = S.seq, 
        contour = TRUE, 
        main = "thermal diffusivity", xlab = "T_water, degC", 
        ylab = "S, -", clab = "m2/s")
image2D(z = Dtp,  x = T.seq, y = P.seq/1000, 
        contour = TRUE, 
        main = "density", xlab = "T_water, degC", 
...
```{r, echo=FALSE}
cp.ref   <- data.frame(  # specific heat capacity, J/m/K 
 temp    = seq(0, 40, by=10),
 sal_10 = c(4141.8, 4136.7, 4132.9, 4130.6, 4129.8),
 sal_30 = c(4019.2, 4022.2, 4024.9, 4027.5, 4030.3))

tc.ref   <- data.frame(  # thermal conductivity, W/kg/K
 temp    = seq(0, 40, by=10),
...
image2D(x = cp.seq, y = por.seq, z = td.por, contour=TRUE,
        las = 1, xlab = "cp, J/kg/K", ylab = "porosity", 
...
image2D(x = Cloud, y = Qrel, z = NLR1, 
        xlab = "Cloudiness", ylab = "Relative humidity", 
...
matplot0D(ToutYr, time_unit = "day",
           ...
           main = "Temperature", xlab = "day", ylab = "degC", 
           lty = 1, lwd = 2)
...
matplot0D(ToutYr2, time_unit = "day",
           ...
           main = "Temperature", ylab = "degC", xlab = "day",
           lty = 1, lwd = 2)
mtext(outer = TRUE, side = 3, "Seasonal variation-submerged")
```
...
The albedo of the sediment is set rather high (0.3) - default = 0.2.

```{r}
nap   <- 0.5   # [m] position of the sampling below NAP
...
```{r, fig.width=10, fig.height=12}
Tdata <- data.frame(var   = paste("Tsed", DataOS$Depth_cm, sep="_0.0"),
                    time  = as.double(DataOS$Time)/86400,
                    value = DataOS$SedimentTemperature)
plot(OSout, mfrow = c(3, 1), which = unique(Tdata$var), time_unit = "day",
     obs = Tdata, obspar = list(pch = ".", col = "red", cex=2))
```

```{r, fig.width=10, fig.height=12,warning = FALSE, message = FALSE}
image2D(OSout, time_origin = "1970-01-01", mfrow = NULL, las = 1)
```
...
image2D(OSout, xlim = as.POSIXct(xlim), 
        ylim = c(1, 0), time_origin = "1970-01-01", 
        xlab = "day", mfrow = NULL)
...
$$
where $K_{bs}$ is the thermal conductivity of the bulk ($W/m/K$), $cp_{bs}$ is the bulk specific heat capacity ($J/kg/K$), and $\rho_{bs}$ is the bulk density ($kg/m3$).
